...
type listSeedsInput struct {
	...
	Status    string `json:"status,omitempty" jsonschema:"事种状态：inbox、doing、done 或 all；省略时为 inbox"`
...
func Handler(db *sql.DB) http.Handler {
	server := mcp.NewServer(
		...
		&mcp.ServerOptions{Instructions: "先调用 list_seeds 按优先级获取待处理事种。处理前调用 start_seed；工作完成后调用 complete_seed。不要处理未成功进入 doing 状态的事种。"},
	)
...
func listSeeds(ctx context.Context, db *sql.DB, input listSeedsInput) ([]seedOutput, error) {
	...
	if status != "all" && status != "inbox" && status != "doing" && status != "done" {
		return nil, errors.New("status 必须是 inbox、doing、done 或 all")
	}
	limit := input.Limit
	if limit == 0 {
